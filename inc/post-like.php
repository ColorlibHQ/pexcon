...
add_action( 'wp_enqueue_scripts', 'pexcon_sl_enqueue_scripts' );
function pexcon_sl_enqueue_scripts() {

	wp_enqueue_script( 'pexcon-post-likes', PEXCON_DIR_JS_URI . 'post-likes.js', array( 'pexcon-ui-js' ), '0.5-s2', true );
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_pexcon_process_simple_like', 'pexcon_process_simple_like' );
add_action( 'wp_ajax_pexcon_process_simple_like', 'pexcon_process_simple_like' );
function pexcon_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'pexcon' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'pexcon' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'pexcon' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'pexcon' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'pexcon' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'pexcon' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'pexcon_show_user_likes' );
add_action( 'edit_user_profile', 'pexcon_show_user_likes' );
function pexcon_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'pexcon' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'pexcon' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
